...
@router.get("/{file_id}/report", response_model=ProjectFileData)
async def get_report_for_file(project_id: int, file_id: int, service: FileService = Depends()):
    """Создать отчет о файле"""
    log.info(f"Received request to to get report for file {file_id} from project {project_id}")
    result = await service.get_report(project_id=project_id, file_id=file_id)
    log.info(f"Retrieved report for file {file_id} from project {project_id}")
    return result
