...
from rest.models.panda_data import LabelData, DefectType ,DefectItem
...
class FileService:
    ...
    async def process_file(self, file_id: int, not_processing: bool = False) -> ProjectFileData:
        ...
        if not file_record.s3_txt_path:
            log.error(f"File {file_id} doesn't have .txt")
            raise HTTPException(status_code=400, detail="File doesn't have .txt")
        ...
        try:
            ...
            # Какой-то сервис, запускающий endpoint "/yolo" и передающий туда текст label из файла
            # Тут же будет его вызов, например, с передачей пути в s3 к файлу .txt
            # Надо только вызвать endpoint и вернуть результат
    ...
    @with_async_db_session
    async def get_report(self, project_id: int, file_id: int) -> ProjectFileData:
        """Создает отчет в формате .pdf с анализом дефектов"""
        ...
        # Загружаем отчет в S3
        s3_report_path = f"reports/{os.path.splitext(file.s3_path)[0]}.pdf"
        s3_report_url = self.s3.upload_file(local_file=pdf_path, s3_file=s3_report_path)

        # Удаляем временный файл и возвращаем результат
        os.remove(pdf_path)
        return (await ProjectFile.update_report_path(
            file_id=file_id,
    ...
    def _create_pdf_document(file, project_id, styles, summary_table, full_table):
        """Создает PDF документ с заданными элементами"""
        ...
        elements = [
            ...
            Paragraph(" ", styles['CyrillicStyle']),
            summary_table,
            Paragraph(" ", styles['CyrillicStyle']),
